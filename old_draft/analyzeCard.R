...
expandVars = unlist(fread("expanded_shortlist.csv",header=F))
...
mixedData = data.frame(expandData[,expandVars])
...
addCoefs = paste(addCoefs,"+",intcpt,"*","ED76",sep="")
 # addCoefs = paste(addCoefs,"+",intcpt,sep="")
}
...
base1 = "LWAGE76~BLACK+EXP76 +EXP762+SMSA76R+REG76R+ED76+KWWb+FAMEDb+LIBCRD14b+MOMDAD14+SINMOM14+STEP14"
base0 = "LWAGE76~BLACK+EXP76 +EXP762+SMSA76R+REG76R+ED76"

summary(lm(base0,data=cardData))
summary(lm(base1,data=cardData))

f3 = formula(paste(base1,addCoefs,sep=""))
f4=formula(paste(base0,addCoefs,sep=""))
cardData$LWAGE76 = as.numeric(cardData$LWAGE76)

varyCoef = lm(f3,data=cardData)
summary(varyCoef)
varyCoef = lm(f4,data=cardData)
summary(varyCoef)
